...
public class CommonLoggingAspect {
	...
	public void loggingAppFlowBefore(final JoinPoint joinPoint) {
	...
	@AfterReturning(pointcut = "com.heyden.teamrelationmanager.aspect.PointcutExpression.controllerDaoService()",
		returning = "result")
	public void loggingAppFlowAfterReturning(final JoinPoint joinPoint, final Object result) {
	...
	@AfterThrowing(pointcut = "com.heyden.teamrelationmanager.aspect.PointcutExpression.controllerPackage()",
		throwing = "exception")
	public void loggingExceptions(final JoinPoint joinPoint, final Exception exception) {
		LOG.error("Exception is thrown on method: " + joinPoint.getSignature().toShortString() + "\n", exception);
	}

	private String getTypeName(final Object object, final String defaultValue) {
